...
assert(
  clampAdminLibraryLimit(200) === ADMIN_LIBRARY_PAGE_SIZE,
  "library list never exceeds page size",
);
...
assert(
  ...
  "exercise validation requires Arabic name and muscle group",
);
...
assert(
  ...
  "approved V2 metadata requires movement role",
);
...
assert(
  ...
  "review-required drafts can be incomplete",
);
...
assert(
  validateProgramDraft({ name_ar: "", slug: "x", duration_weeks: 12, days_per_week: 4 }).name_ar,
  "program name required",
);
assert(
  canPublishContent({ title: "عنوان", short_description: "ملخص كاف" }),
  "content publish needs title+summary",
);
assert(
  !canPublishContent({ title: "عنوان", short_description: "" }),
  "content publish blocked without summary",
);
assert(
  validateContentDraft({
    title: "x",
  ...
  "draft save does not require summary",
);
...
assert(
  overlaidMeals.length === 1 && overlaidMeals[0]?.name === "db",
  "DB wins and archived seed is hidden",
);
...
assert(
  overlaidDiscover[0]?.title === "db" && overlaidDiscover[0]?.coverImage === "seed.png",
  "DB title wins, seed cover kept",
);

assert(
  translateLibraryError({ message: "allergens_review_required" }).includes("الحساسية"),
  "known SQL errors are Arabic",
);
assert(CMS_SOURCE_OF_TRUTH === "database_with_seed_fallback", "CMS SoT is DB with seed fallback");
assert(
  PROGRAM_VERSIONING_COMPLETION_REQUIRED,
  "program snapshot versioning is not claimed complete",
);
...
assert(
  migration.includes("REVOKE DELETE ON public.discover_content"),
  "discover hard delete revoked",
);
assert(
  migration.includes("LEAST(GREATEST(COALESCE(p_limit, 25), 1), 50)"),
  "library pagination is clamped",
);
assert(
  migration.includes("exercise_archived") || migration.includes("exercise_published"),
  "exercise status is audited",
);
...
assert(
  migration.includes("allergens_review_required"),
  "allergen review is required after ingredient edits",
);
...
const exerciseUi = readFileSync(
  join(root, "src/components/admin/libraries/ExerciseLibraryManager.tsx"),
  "utf8",
);
assert(
  !exerciseUi.includes("hard delete") && !exerciseUi.includes('.from("exercises").delete'),
  "exercise UI has no hard delete",
);
...
const mealUi = readFileSync(
  join(root, "src/components/admin/libraries/NutritionLibraryManager.tsx"),
  "utf8",
);
assert(mealUi.includes("uploadAdminMealImage"), "new meal supports a real image upload");
assert(mealUi.includes("cc-meal-client-preview"), "meal opens in a client-style live preview");
assert(mealUi.includes('openItem("new")'), "new meal button opens the working editor");
...
const programUi = readFileSync(
  join(root, "src/components/admin/libraries/ProgramLibraryManager.tsx"),
  "utf8",
);
assert(programUi.includes("التعيينات الحالية تبقى كما هي"), "template ≠ assignment is shown");
assert(programUi.includes("PROGRAM_VERSIONING_COMPLETION_REQUIRED"), "versioning gap is disclosed");
const programBuilderUi = readFileSync(
  join(root, "src/components/admin/programs/AdminProgramBuilder.tsx"),
  "utf8",
);
...
const contentUi = readFileSync(
  join(root, "src/components/admin/libraries/ContentLibraryManager.tsx"),
  "utf8",
);
assert(contentUi.includes("محتوى جديد"), "content list can create");
const contentBuilderUi = readFileSync(
  join(root, "src/components/admin/content/AdminContentBuilder.tsx"),
  "utf8",
);
assert(contentBuilderUi.includes("حفظ مسودة"), "content save is draft");
assert(contentBuilderUi.includes("نشر على التطبيق"), "publish is a separate action");
...
assert(contentBuilderUi.includes("الكل"), "audience can target everyone");
assert(
  contentBuilderUi.includes("صور المنشور") || contentBuilderUi.includes("إضافة / تغيير"),
  "multi-image cover upload exists",
);
assert(
  contentBuilderUi.includes("أولوية الظهور") || contentBuilderUi.includes("يظهر في الرئيسية"),
  "content placement controls exist",
);
assert(contentUi.includes("أعلى") && contentUi.includes("أسفل"), "list can reorder priority");
assert(
  contentUi.includes("في الرئيسية") || contentUi.includes("featured"),
  "list can place content on home",
);
assert(
  contentUi.includes("حذف نهائي") ||
    contentUi.includes("confirmDeleteDb") ||
    contentUi.includes("deleteAdminDiscoverContent"),
  "content can be permanently deleted",
);
assert(
  contentUi.includes("ترتيب") || contentUi.includes("الأحدث تحديثاً"),
  "content list supports sorting",
);
assert(
  contentUi.includes("cc-cms-tile__hover") &&
    contentUi.includes("تعديل") &&
    contentUi.includes("حذف"),
  "content tiles expose hover edit/delete/archive",
);
...
const contentPreviewUi = readFileSync(
  join(root, "src/components/admin/content/AdminContentAppPreview.tsx"),
  "utf8",
);
assert(
  contentPreviewUi.includes("DiscoverContentDetailView"),
  "preview uses the real discover screen",
);
assert(contentPreviewUi.includes("HomeDiscoverCard"), "preview uses the real home card");
assert(
  contentPreviewUi.includes("تفاصيل المحتوى") ||
    contentPreviewUi.includes("DiscoverContentDetailView"),
  "preview matches client discover screen",
);
