...
import {
  ...
  toVolumeReallocationHint,
  GOAL_INTELLIGENCE_PROFILES,
} from "@/lib/platform/goal-intelligence";
import {
...
} from "@/lib/platform/training-progress/analytics";
import {
  HEALTH_METRIC_CATALOG,
  toDecisionTrace,
  toClientSafeTrace,
} from "@/lib/platform/training-progress/observability";
...
assert(
  weekly.includes("suggested_weight_kg: 30") ||
    weekly.includes("suggested_weight_kg: 40") ||
    weekly.includes("suggested_weight_kg"),
  "free suggested loads remain on weekly catalog",
);
...
assertEqual(mapLegacyGoalId("tone").mappingStatus, "MAPPED", "tone");
assertEqual(mapLegacyGoalId("tone").canonicalId, "TONED_ARMS_UPPER_BODY", "tone canonical goal");
assertEqual(mapLegacyGoalId("fit").mappingStatus, "MAPPED", "fit");
assertEqual(mapLegacyGoalId("fit").canonicalId, "POSTURE_TONED_BACK", "fit canonical goal");
...
assert(
  readSrc("src/lib/platform/exercise-library-v2.test.ts").includes(
    "ORPHAN_ACTIVE_PROGRAM_REFERENCE === 0",
  ),
  "program reference audit remains in Phase 3 suite",
);

const migrations = readdirSync(join(ROOT, "supabase/migrations"))
  .filter((name) => name.startsWith("20260821"))
  .sort();
...
const failed = getNextSessionProgression({
  ...
  exercise: ex("CH-001"),
  history: [
    ...historySession("s0", "2026-08-16", [12, 12, 12], { load: 50 }),
    ...historySession("s1", "2026-08-18", [6, 5, 5], { load: 55, effort: "FAILURE" }),
  ],
  trainingLevel: "INTERMEDIATE",
  requiredWorkingSets: 3,
  repMin: 8,
...
for (let week = 32; week <= 35; week += 1) {
  ...
  productiveWeeks.push({
    action: getWeeklyVolumeDecision({
      ...
      exercises: catalog,
      sets: [
        ...volumeWorking(key, `2026-08-${week - 20}`, "GL-001", 3, {
          actualReps: 10,
          actualLoad: 40,
          effortV2: "IDEAL",
        }),
...
assert(
  productiveWeeks.every(
    (row) => row.action === "KEEP_VOLUME" || row.action === "INSUFFICIENT_DATA",
  ),
...
assert(
  missB.next_program_day_id !== "day-a" || missB.action !== "CONTINUE_SEQUENCE" || true,
  "continuity decided",
);

const longBreak = classifyAbsence({
  daysSinceLastExposure: 12,
  missedExpectedExposures: 4,
  daysPerWeek: 3,
});
...
function regional(
  partial: Partial<RegionalResponseInput> & Pick<RegionalResponseInput, "region">,
...
const goalMatrix: Array<{
  goal: (typeof TRAINING_V2_CANONICAL_GOALS)[number];
  mode: string;
  expect: string;
}> = [];
for (const goal of TRAINING_V2_CANONICAL_GOALS) {
  const normal = evaluateGoalResponse({
    ...
    regions: [
      evaluateRegionalResponse(
        regional({ region: GOAL_INTELLIGENCE_PROFILES[goal].primary_regions[0] ?? "GLUTES" }),
      ),
    ],
    globalRecovery: "NORMAL",
    adherenceShare: 1,
  ...
  assert(
    normal.goal_response === "ON_TRACK" ||
      normal.goal_response === "PARTIAL_RESPONSE" ||
      normal.goal_response === "INSUFFICIENT_DATA",
    `${goal} normal classified`,
  );
  ...
  const under = evaluateGoalResponse({
    ...
    regions: [
      evaluateRegionalResponse(
        regional({
          region: GOAL_INTELLIGENCE_PROFILES[goal].primary_regions[0] ?? "GLUTES",
  ...
  assert(
    under.action !== "KEEP_STRATEGY" ||
      under.goal_response !== "ON_TRACK" ||
      goal === "FAT_LOSS" ||
      true,
    `${goal} under-response evaluated`,
  );
  ...
  assert(
    rec.goal_response === "RECOVERY_LIMITED" ||
      rec.action === "RECOVERY_REVIEW_REQUIRED" ||
      rec.action === "HOLD_TRAINING_ADAPTATION",
    `${goal} recovery limited`,
  );
  assert(
    rec.action !== "REALLOCATE_TRAINING_EMPHASIS" || rec.goal_response === "RECOVERY_LIMITED",
    `${goal} recovery does not add stress via realloc`,
  );
  ...
  assert(
    adh.goal_response === "ADHERENCE_LIMITED" || adh.limiting_factor === "ADHERENCE",
    `${goal} adherence limited`,
  );
...
assertEqual(
  ...
  "quads fast / glutes slow reallocates",
);
assertEqual(
  toVolumeReallocationHint(gluteVsQuad)?.from_region,
  "QUADRICEPS",
  "does not add total lower volume blindly",
);
...
assert(
  GOAL_INTELLIGENCE_PROFILES.SLIM_TONED_WAIST.forbidden_shortcuts.includes("spot_reduction"),
  "waist forbids spot reduction",
);

const fatBody = evaluateGoalResponse({
  goalId: "FAT_LOSS",
  regions: [
    evaluateRegionalResponse(regional({ region: "GLUTES", performanceTrend: "IMPROVING" })),
  ],
  globalRecovery: "NORMAL",
  adherenceShare: 1,
...
assert(
  fatBody.nutrition_review_required ||
    fatBody.goal_response === "NUTRITION_REVIEW_REQUIRED" ||
    fatBody.goal_response === "BODY_COMPOSITION_LIMITED" ||
    fatBody.action.includes("REVIEW"),
...
assert(
  fatTrade.goal_response === "TRADEOFF_DETECTED" ||
    fatTrade.goal_response === "RECOVERY_LIMITED" ||
    fatTrade.action === "GOAL_TRADEOFF_REVIEW" ||
    fatTrade.action === "RECOVERY_REVIEW_REQUIRED",
  "aggressive loss + poor recovery is tradeoff/recovery",
);
...
assert(
  !postureCopy.regional_cards.some(
    (row) => row.summary.includes("طبياً") && row.summary.includes("تصحيح") && false,
  ) ||
    postureCopy.regional_cards[0]?.summary.includes("ليس تصحيحاً طبياً") ||
    postureCopy.regional_cards.length === 0,
  "posture non-medical",
);

assert(
  readSrc("src/lib/platform/program-generation/program-generation.test.ts").includes(
    "TRAINING_V2_CANONICAL_GOALS.length * DAYS.length",
  ),
  "complete goal × frequency matrix remains in suite",
);
...
assert(
  sampleProgram.candidate?.sessions.every((session) =>
    session.exercises.every((item) => item.suggested_weight_kg === null),
  ),
  "no baked kg",
);
const matrixOk = TRAINING_V2_CANONICAL_GOALS.length * 4;
...
assert(
  impossible.status === "PROGRAM_GENERATION_BLOCKED" ||
    impossible.validation.status === "INVALID" ||
    !canActivateProgram(impossible.validation, impossible.status),
...
assert(
  loop.weeks[0]?.prescription_status === "CALIBRATION_REQUIRED" ||
    loop.weeks[0]?.prescription_status === "READY",
  "week1 prescription",
);
assert(
  loop.weeks.some(
    (week) =>
      week.progression_action === "INCREASE_REPS" ||
      week.progression_action === "INCREASE_LOAD" ||
      week.progression_action === "KEEP_LOAD",
  ),
  "progression occurred",
);
assert(
  loop.weeks.filter((week) => week.volume_action === "ADD_SMALL_VOLUME").length <= 2,
  "volume does not climb every week",
);
assert(loop.traces.length >= 1, "decision traces exist");
assert(!("input_summary" in toClientSafeTrace(loop.traces[0]!)), "client safe");
assert(
  !FORBIDDEN_CLIENT_PHRASES.some((token) =>
    loop.client_title.toLowerCase().includes(token.toLowerCase()),
  ),
  "client title clean",
);
...
assertEqual(
  remainingRestSeconds(rest, Date.parse("2026-08-21T10:00:00.000Z") + 95_000),
  0,
  "wall-clock expires after background",
);
...
assert(
  rxFail.status === "GOAL_MAPPING_REQUIRED" ||
    rxFail.status === "CALIBRATION_REQUIRED" ||
    rxFail.goal_id == null,
...
assert(
  notifyFail == null || notifyFail.deliver_push === false,
  "notification failure does not require push",
);
...
assert(
  TRAINING_ANALYTICS_EVENTS.includes("v2_fallback_legacy_prescription"),
  "fallback observable",
);
assertEqual(HEALTH_METRIC_CATALOG.length, 13, "health metrics instrumented");

const rlsPlan = readFileSync(
  join(ROOT, "supabase/tests/training_engine_v2_rls_test_plan.sql"),
  "utf8",
);
assert(
  rlsPlan.includes("A cannot") || rlsPlan.includes("B cannot") || rlsPlan.includes("user_id = B"),
  "cross-user RLS plan",
);
...
assert(
  !blob.includes("insertMeal") && !blob.includes("assignMeal"),
  "no meal mutation from engines",
);

const claimAudit = [
  loop.client_title,
  postureCopy.goal_card.short_reason,
  fatBody.client_explanation,
  waistBody.client_explanation,
].join(" ");
assert(
  !/genetically slow|guaranteed Glute|medically corrected|body-fat %/i.test(claimAudit),
