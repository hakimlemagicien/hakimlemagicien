...
export const GOAL_INTELLIGENCE_PROFILES: Record<TrainingV2CanonicalGoal, GoalIntelligenceProfile> = {
  ...
  MUSCLE_GROWTH: {
    goal_id: "MUSCLE_GROWTH",
    primary_regions: GOAL_MUSCLE_PROFILES.MUSCLE_GROWTH.primary,
    secondary_regions: GOAL_MUSCLE_PROFILES.MUSCLE_GROWTH.secondary,
    maintenance_regions: GOAL_MUSCLE_PROFILES.MUSCLE_GROWTH.maintenance,
    training_success_signals: ["chest_back_shoulder_quad_progression", "regional_volume", "adherence", "recovery"],
    body_composition_dependency: "OPTIONAL_CONFIRMATION",
    protected_outcomes: ["training_recovery", "push_pull_balance"],
    regional_under_response_rule: "If one primary lags with recoverable budget, reallocate — do not add total volume.",
    program_limiter_rule: "Missing push/pull coverage is PROGRAM_LIMITED, not a hypertrophy failure.",
    body_composition_limiter_rule: "Scale weight is not muscle-growth confirmation.",
    nutrition_review_rule: "Strength up with no size change may need nutrition surplus review. No calorie writes.",
    conflict_rules: ["aggressive_cut_vs_hypertrophy"],
    allowed_actions: KEEP_SET,
    forbidden_shortcuts: ["add_total_volume", "judge_by_scale_only", "bro_split_max_pump"],
  },
  FITNESS_ENERGY: {
    goal_id: "FITNESS_ENERGY",
    primary_regions: GOAL_MUSCLE_PROFILES.FITNESS_ENERGY.primary,
    secondary_regions: GOAL_MUSCLE_PROFILES.FITNESS_ENERGY.secondary,
    maintenance_regions: GOAL_MUSCLE_PROFILES.FITNESS_ENERGY.maintenance,
    training_success_signals: ["resistance_adherence", "recoverable_training", "balanced_coverage"],
    body_composition_dependency: "OPTIONAL_CONFIRMATION",
    protected_outcomes: ["training_recovery", "resistance_quality"],
    regional_under_response_rule: "Do not specialize one region because energy/fitness feels flat.",
    program_limiter_rule: "Balanced coverage supports fitness. Missing roles are PROGRAM_LIMITED.",
    body_composition_limiter_rule: "Energy is not proven by scale weight.",
    nutrition_review_rule: "Persistent fatigue with good training → NUTRITION/RECOVERY review. No calorie writes.",
    conflict_rules: ["overreach_vs_general_fitness"],
    allowed_actions: KEEP_SET,
    forbidden_shortcuts: ["cardio_only_fitness", "add_sets_for_energy"],
  },
  ATHLETIC_PHYSIQUE: {
    goal_id: "ATHLETIC_PHYSIQUE",
    primary_regions: GOAL_MUSCLE_PROFILES.ATHLETIC_PHYSIQUE.primary,
    secondary_regions: GOAL_MUSCLE_PROFILES.ATHLETIC_PHYSIQUE.secondary,
    maintenance_regions: GOAL_MUSCLE_PROFILES.ATHLETIC_PHYSIQUE.maintenance,
    training_success_signals: ["balanced_regional_progression", "adherence", "recovery"],
    body_composition_dependency: "OPTIONAL_CONFIRMATION",
    protected_outcomes: ["regional_balance", "training_recovery"],
    regional_under_response_rule: "One region FAST vs others does not auto-suppress unless recovery requires reallocation.",
    program_limiter_rule: "If one region has no coverage, PROGRAM_LIMITED rather than imbalance biology.",
    body_composition_limiter_rule: "Scale weight is not athletic-physique success.",
    nutrition_review_rule: "Body-composition conflict with athletic look → TRADEOFF/NUTRITION review.",
    conflict_rules: ["single_region_dominance_vs_balance"],
    allowed_actions: KEEP_SET,
    forbidden_shortcuts: ["one_region_maximum", "judge_by_scale_only"],
  },
  BODY_RESHAPE: {
    goal_id: "BODY_RESHAPE",
    primary_regions: GOAL_MUSCLE_PROFILES.BODY_RESHAPE.primary,
    secondary_regions: GOAL_MUSCLE_PROFILES.BODY_RESHAPE.secondary,
    maintenance_regions: GOAL_MUSCLE_PROFILES.BODY_RESHAPE.maintenance,
    training_success_signals: ["performance_preserved", "resistance_adherence", "recoverable_training"],
    body_composition_dependency: "REQUIRED_FOR_FULL_CONFIRMATION",
    protected_outcomes: ["strength_preservation", "training_recovery"],
    regional_under_response_rule: "Do not add resistance sets because the mirror is unchanged.",
    program_limiter_rule: "Balanced resistance supports reshape. Visual change is not workout-calorie success.",
    body_composition_limiter_rule: "Training good + shape stagnant → NUTRITION/BODY_COMPOSITION review.",
    nutrition_review_rule: "Shape change with declining performance → TRADEOFF + NUTRITION_REVIEW_REQUIRED.",
    conflict_rules: ["aggressive_cut_vs_performance_preservation"],
    allowed_actions: KEEP_SET,
    forbidden_shortcuts: ["infer_reshape_from_workout_calories", "spot_reduction"],
  },
  HEALTHY_WEIGHT_GAIN: {
    goal_id: "HEALTHY_WEIGHT_GAIN",
    primary_regions: GOAL_MUSCLE_PROFILES.HEALTHY_WEIGHT_GAIN.primary,
    secondary_regions: GOAL_MUSCLE_PROFILES.HEALTHY_WEIGHT_GAIN.secondary,
    maintenance_regions: GOAL_MUSCLE_PROFILES.HEALTHY_WEIGHT_GAIN.maintenance,
    training_success_signals: ["chest_back_shoulder_quad_progression", "regional_volume", "adherence", "recovery"],
    body_composition_dependency: "REQUIRED_FOR_FULL_CONFIRMATION",
    protected_outcomes: ["training_recovery", "lean_gain_quality"],
    regional_under_response_rule: "If primaries lag with recoverable budget, reallocate — do not add junk volume.",
    program_limiter_rule: "Missing compound coverage is PROGRAM_LIMITED, not a surplus failure.",
    body_composition_limiter_rule: "Scale-up without training stimulus → BODY_COMPOSITION/NUTRITION review, not more isolation.",
    nutrition_review_rule: "Gain is nutrition-owned. Engine does not write calories.",
    conflict_rules: ["surplus_vs_recovery"],
    allowed_actions: KEEP_SET,
    forbidden_shortcuts: ["dirty_bulk_as_success", "add_total_volume", "judge_by_scale_only"],
  },
};
