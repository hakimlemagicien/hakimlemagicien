...
export type BillingSnapshot = {
  plan: PaidTierId | "free" | "admin" | string;
...
export async function fetchMyBilling(): Promise<BillingSnapshot | null> {
  ...
  const months = row.billing_period_months === 6 || row.billing_period_months === 3
  ...
  return {
    ...
    paidPeriodEnd: typeof row.paid_period_end === "string" ? row.paid_period_end : (typeof row.ends_at === "string" ? row.ends_at : null),
    ...
    personalProgramDeliveredAt: typeof row.personal_program_delivered_at === "string" ? row.personal_program_delivered_at : null,
  };
}

export async function cancelMyRenewal() {
  const { data, error } = await supabase.rpc("cancel_my_renewal");
  if (error) throw error;
  return data as { paid_period_end?: string; cancel_at_period_end?: boolean };
}
