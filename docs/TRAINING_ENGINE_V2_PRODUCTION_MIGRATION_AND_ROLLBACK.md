...
Until a clone exists, PF-2 remains **SCHEMA_CHAIN_VERIFIED_ON_STAGING; PRODUCTION_CLONE_NOT_AVAILABLE**.

Staging `dxerwrdpcflpnjvsnrjq` already applied the `4d80f8d` chain (Database Gate). That is **not** a substitute for a Production dump restore. Do not dump live Production without CEO order.

### has_role() Production note

Staging PostgREST/JWT requires:

```sql
GRANT EXECUTE ON FUNCTION public.has_role(UUID, public.app_role) TO authenticated;
```

RLS policies call `has_role()`. Revoking EXECUTE breaks own-profile and own-training table reads. This is **not** a weakening of row policies. Production apply must include this grant (or rewrite policies to stop calling `has_role` from the `authenticated` invoker). Do not silently omit it.
...
| Gate | Status |
|------|--------|
| PF-2 dry-run | `SCHEMA_CHAIN_VERIFIED_ON_STAGING` — Production dump clone **not** authorized/executed |
| PF-3 rollback plan | `DOCUMENTED` — restore-backup primary; no Production apply |
| Production migrations | **Not applied** |
| `has_role` EXECUTE for `authenticated` | **Required** on Staging JWT path; include in Production apply review |

### Owners (PF-3 — do not execute)

| Event | Owner |
|-------|--------|
| Pre-deploy Production backup | Database Architect |
| Stop on migration failure | Database Architect |
| Backup restore | Database Architect + CEO |
| App rollback to last Production SHA | Platform Developer (no `--prod` until CEO) |
| Partial deploy (schema yes / app no) | Hold merge to `main` |
