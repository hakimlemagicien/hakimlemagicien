...
| **PF-2** | Migration dry-run on a production-like copy | Database Architect | Schema chain verified on Staging; **Production dump clone not executed** (no live dump authorized) |
| **PF-3** | Production migration apply + rollback plan | Database Architect + CEO | Plan documented — **do not apply** |
| **PF-4** | Canonical durable Staging URL | Platform Developer / Vercel login + DNS | **BLOCKED** — Vercel CLI logged out; `staging.hakimlemagicien.com` not bound |
...
## 6b. Goal source of truth

| Surface | Source |
|---------|--------|
| Quiz / onboarding | `profiles.goal` (legacy id: `fat`, `glutes`, …) |
| Profile UI | `profiles.goal` |
| Training Engine / Phase 9 / Generator | `resolveCanonicalGoal(profiles.goal)` → `TRAINING_V2_*` |
| Program assignment snapshot | candidate `goal_id` must match canonical mapping |
| Workout hero | **must** render `trainingGoalLabelAr(profiles.goal)` — never a hardcoded FAT_LOSS title |

A Staging assignment of `GLUTE_GROWTH` while the workout hero showed «خسارة الدهون» was a **UI defect** (hardcoded marketing title), not two engines with two goals. Closure commit binds the hero to the profile mapping.
