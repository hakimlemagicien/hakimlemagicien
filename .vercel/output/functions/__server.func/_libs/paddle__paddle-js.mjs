//#region node_modules/@paddle/paddle-js/dist/index.esm.js
...
var PaddleClassicInfo = { url: PaddleClassicCDNUrl };
var PaddleBillingV1Info = { url: PaddleBillingCDNUrl };
...
function injectScript(src) {
	...
	if (!headOrBody) throw new Error("Cannot inject Paddle.js. It needs a <head> or <body> element.");
...
function loadFromCDN(version) {
	...
	promiseMap[version] = new Promise(function(resolve, reject) {
		...
		try {
			...
			if (!script) script = injectScript(cdnUrl);
			script.addEventListener("load", function() {
				if (window[paddleInstanceName]) resolve(window[paddleInstanceName]);
				else reject(/* @__PURE__ */ new Error("Paddle.js not available"));
			});
...
function getCDNInfoBasedOnVersion(version) {
	if (version === Versions.CLASSIC) return PaddleClassicInfo;
	if (version === Versions.V1) return PaddleBillingV1Info;
	else {
		console.error("[Paddle] Unknown Paddle Version");
		return;
	}
}
var _excluded = ["environment", "version"], _excluded2 = ["environment", "version"];
function initializePaddleBillingV1(options, paddle) {
	...
	try {
		...
		if (paddle.Initialized) paddle.Update(_objectSpread2({}, rest));
		else paddle.Initialize(_objectSpread2({}, rest));
	} catch (e) {
		...
		return _regeneratorRuntime().wrap(function _callee$(_context) {
			while (1) switch (_context.prev = _context.next) {
				...
				case 3:
					...
					if (options) {
						if (requestedVersion === Versions.V1) initializePaddleBillingV1(options, paddle);
						else if (requestedVersion === Versions.CLASSIC) initializePaddleClassic(options, paddle);
					}
